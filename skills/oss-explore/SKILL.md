...
스크립트는 설치된 스킬 루트의 `scripts/`에서 실행한다. 터미널 출력이 기본이며 자동화에는 `--json`, 비교용 보고서에는 `--html`을 쓴다.
...
4. 추천이나 채택 판단이면 상위 3~5개의 README, 최신 릴리스, community profile을 추가 확인한다.
5. 기여가 목적이면 `explore.sh "<주제>" --issues` 또는 `discover.sh`로 전환한다.
