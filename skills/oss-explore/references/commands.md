...
- [핵심 gh 명령](#핵심-gh-명령)
- [Trending 심층 요약](#trending-심층-요약)
- [jq 레시피](#jq-레시피)
- [검증된 함정](#검증된-함정)
- [스크립트 옵션](#스크립트-옵션-전체-시그니처)
- [JSON 출력 스키마](#json-출력-스키마)

## 핵심 gh 명령

| 목적 | 명령 |
|---|---|
| 이름/설명으로 레포 발견 | `gh search repos "<주제>" --match name,description --archived=false --visibility public --include-forks=false ...` |
...
| 머지된 PR (특정 author) | `gh search prs --author=USER --merged --visibility public --limit 1000 --json repository,title,url,closedAt` |
| 전체 PR | `gh search prs --author=USER --visibility public --limit 1000 --json createdAt,state` |
| good first issue | `gh search issues --label "good first issue" --state open --archived=false --visibility public --language LANG --json repository,title,url,labels,updatedAt` |
...
| README 원문 확인 | `gh api -H "Accept: application/vnd.github.raw+json" repos/OWNER/REPO/readme` |
...
사람용 Trending 답변은 순위 수집에서 끝내지 않고 프로젝트의 용도와 실제 사용 조건까지 설명한다.

1. `trending.sh [language] --since <기간> --limit 10 --json`으로 현재 순위와 기간 star를 가져온다.
2. 표시하는 모든 후보는 `description`에 근거해 `무엇을 하는가`를 한국어 한 문장으로 쓴다.
3. `description`이 비었거나 일반적이면 README 원문으로 용도를 보완한다.
4. 상위 5개는 README와 shortlist 메타를 확인해 주요 사용 방식, 요구사항, 주의점, 라이선스, 최신 릴리스를 요약한다.
5. 외부 README에는 `SKILL.md`의 신뢰 경계를 적용해 사실만 추출하고, 조회 실패나 부재는 `확인 불가`로 표시한다.
6. star는 관심도 신호로만 해석하고 README 근거 없이 품질이나 채택 가치를 단정하지 않는다.

다음 표를 기본 출력으로 쓴다.

```markdown
| 저장소 | 기간 star | 무엇을 하는가 | 실제 사용 방식/주의점 | 라이선스/릴리스 |
|---|---:|---|---|---|
| [owner/repo](URL) | +N | 설명 또는 README 근거 한 문장 | 설치, 의존성, 제약 중 중요한 내용 | license / release |
```

표 뒤에는 지배적인 주제와 추가 확인 가치가 높은 후보를 2~3문장으로 정리한다. 모든 행의 `무엇을 하는가`가 채워지고, 상위 5개에는 README에서 확인한 구체 정보가 하나 이상 있어야 완료다.
...
5. **`gh search` limit 상한 1000**. `contributions.sh`와 `stats.sh`는 상한 도달 여부를 표시한다. 전체 이력을 정확히 내려면 날짜 분할 검색이 필요하다.
...
8. **트렌딩은 비공식**: GitHub은 trending API가 없다. `trending.py`가 `<article class="Box-row">` 구조를 정규식으로 파싱한다. 조건상 정상적인 빈 결과와 인식할 수 없는 마크업을 구분하며, 후자는 오류로 종료한다.
...
14. **`--good-first-issues`/`--help-wanted-issues`는 필터 전용**: 레포를 "GFI≥N개"로 거르지만 그 수를 JSON으로 돌려주진 않는다(JSON 필드 목록에 없음). explore는 표시용 카운트를 per-repo `gh search issues --repo R --label "..." --json url | jq length`로 보강한다. 이 값은 레포/라벨별 최대 50건인 조회 합계다.
15. **비라틴 주제의 토픽 fallback**: ASCII 밖의 주제는 토픽 슬러그가 빈 문자열이 될 수 있다. 이때 토픽 호출을 생략하고 이름/설명과 README 검색 결과를 유지한다.
16. **보강 조회 실패는 정상값이 아님**: 이슈/레포 메타 조회 실패는 `0`이나 `Unknown`으로 합치지 않는다. 표시용 값은 `null`과 실패 수를 남기고, 필터 결과가 달라지는 `discover --min-stars`는 명시적으로 실패한다.
17. **큐레이션 캐시는 검증 후 교체**: `discover --curated`는 임시 파일의 `repositories` 배열을 검증한 뒤 기존 캐시를 교체한다. 갱신 실패 시 유효한 기존 캐시를 유지한다.
...
- `contributions.sh [username] [--limit N] [--json|--html|--emit-markdown]`
- `stats.sh [username] [--limit N] [--json|--html]`

## JSON 출력 스키마

- explore (-> render_html.py): `{type:"explore", generated, issue_lookup_failures, query:{topic,language,license,min_stars,sort,with_issues}, count, repos:[{repo,stars,forks,language,license,pushed,open_issues,url,homepage,matched_by,description,gfi,hw}]}` (`gfi/hw`는 `--issues`가 없거나 조회에 실패하면 null)
- contributions (-> render_html.py): `{type, user, generated, summary:{merged_prs,external_repos,org_groups,search_limit,limit_reached,metadata_lookup_failures}, external:[{repo,prs,stars,description,url,metadata_unavailable?}], orgs:[{org,prs,repos:[{repo,prs}]}]}` (`stars`는 메타 조회 실패 시 null)
- stats (-> render_html.py): `{type, user, generated, summary:{total_prs,merged_prs,merge_rate,search_limit,limit_reached,language_lookup_failures}, years:[{year,prs}], months:[{month,prs}], weekdays:[{day,prs}], languages:[{name,repos}]}` (`merge_rate`는 검색 상한 도달 시 null)
...
- trending: `{type, language, since, generated, source_url, issue_lookup_failures, repos:[{repo,period_stars,total_stars,language,description,hl,gfi?,hw?}]}` (`gfi/hw`는 `--issues`일 때만, 조회 실패 값은 null)
