...
- `apple-stt` context는 CLI 경계에서 한 번만 고른다. 인자 없는 직접 실행은 `~/.config/stt/vocab.txt` 상위 100개를 쓰고, Voice Memos는 녹음별 context pack을 `--vocab-file`로 전달해 기본값을 대체한다.
- `summarize.py`는 claude-agent-sdk 의존이라 이 디렉터리의 uv venv로 실행한다(`uv run`). Summary와 correction 모두 `ClaudeAgentOptions(tools=[])`로 CLI `--tools ""`를 내보내야 한다. 빈 `allowed_tools`는 도구를 끄지 않는다.
- `stt_fallback.swift`와 tsrp/UI trigger는 비실행 legacy reference다. `SpeechTranscriber` 외의 전사 fallback으로 호출하지 않는다.
