...
argument-hint: "[mode]"
name: tts
description: "텍스트를 음성으로 합성(TTS)하고 내 목소리를 복제(voice clone). 로컬·오프라인, 기본 엔진 Qwen3-TTS(mlx-audio, 오픈웨이트). 3초 샘플로 음성 복제, full/chunk 두 모드 + 청크 단위 부분 재생성. 사용자가 'TTS', '텍스트 음성으로', '음성 합성', '음성 복제', '보이스 클론', 'voice clone', '내 목소리로', '성우 음성', '더빙', '나레이션 만들어', 'Qwen3-TTS', '음성 생성' 등을 언급하면 사용. 빠른 비복제 음성은 macOS say. (음성→텍스트 전사·받아쓰기 STT는 stt 스킬.)"
---
...
python3 "$S" prep  <영상/음성파일> --voice myvoice [--ss 9.6 --dur 14]  # 목소리 1회 등록(보관함)
python3 "$S" voices                                                   # 등록된 목소리 목록
python3 "$S" preptext --text-file script.md --out script-tts.txt      # 한국어 발화/음차/청크 전처리
python3 "$S" chunk --voice myvoice --text-file script.txt               # → /tmp/tts-XXXX/output.wav
python3 "$S" chunk --voice myvoice --text-file script.txt --out ~/Desktop/intro.wav  # 보관 위치 지정
python3 "$S" chunk --voice myvoice --text-file script.txt --out out/raw.wav --loudnorm-out out/edit.wav  # 원본+편집용
...
python3 "$S" full  --voice myvoice --text-file script.txt               # 한 방 생성
```
