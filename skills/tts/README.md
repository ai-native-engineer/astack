...
사람의 음성·전사문은 개인 데이터이므로 shared 스킬이나 플러그인 repo에 추가하지 않는다. 기본 로컬 저장 권한은 음성/작업 디렉터리 `700`, 개인정보 파일 `600`이다. 모델 가중치만 Hugging Face 캐시에 내려받고 음성·대본은 원격 추론 서비스로 전송하지 않는다.
