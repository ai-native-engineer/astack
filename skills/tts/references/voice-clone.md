...
- 드라이버가 생성·관리하는 음성 보관함과 작업 프로젝트는 `700`, `ref.wav`, `ref.txt`, `manifest.json`, 생성 WAV는 `600` 권한으로 저장한다.
- 입력 미디어·대본·지시 파일은 읽기만 하며 권한을 바꾸지 않는다.
- 기존 음성 보관함은 `voices`로 조회하거나 해당 음성을 사용할 때 같은 권한으로 보정한다. 보관함이나 음성 파일이 심볼릭 링크면 외부 파일 권한을 바꾸지 않고 거부한다.
...
- 음성 이름은 경로가 아닌 단순 별칭을 쓴다. 같은 이름이 있으면 `prep`은 중단하며, 교체하려는 경우에만 `--replace`를 명시한다.
- `--replace`는 변환과 전사가 모두 성공한 뒤 음성 폴더 전체를 교체하고, 교체 실패 시 기존 폴더를 복구한다.
