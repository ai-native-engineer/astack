...
- headless 실행(`codex exec` 등)으로 대체하지 않는다. pane 점유가 방해라는 판단으로 강등하지도 않는다. 관찰 가능한 pane과 turn 간 맥락 유지가 이 transport의 목적이다.
- worker마다 별도 pane과 이름을 사용한다.
- `start`가 결과 파일과 완료 sentinel 규칙을 prompt에 자동으로 붙인다. agent 명령이 codex면 `-c notify=[...]`(`scripts/zdel-notify`), claude면 `--settings`의 Stop hook(`scripts/zdel-notify-claude`)도 자동 추가해, 모델이 파일 지시를 무시해도 turn 완료 시 harness가 `.done`을 만들고 마지막 메시지를 결과 파일로 저장한다.
- `wait`는 sentinel과 함께 pane 화면 정지를 감시한다. TUI agent는 작업 중 매초 스피너를 다시 그리므로, 화면이 일정 시간(기본 12초, `ZDEL_IDLE_AFTER`) 그대로면 sentinel이 없어도 pane tail을 출력하고 exit 3으로 즉시 반환한다. notify가 없는 harness의 worker가 채팅으로만 답했거나, 프롬프트 미제출/dialog 상태라는 뜻이다.
...
timeout(exit 124)이면 `peek`으로 먼저 확인한다. 작업 중이면 wait 시간을 늘리고, 입력 대기면 `key` 또는 `send`로 같은 pane을 복구한다. exit 3(idle without sentinel)이면 함께 출력된 pane tail에서 결과를 바로 회수하거나, 같은 worker에 `send`로 해당 turn 파일 저장만 재요청한다.
