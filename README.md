...
| `tts` | Local text-to-speech & voice cloning (Qwen3-TTS via mlx-audio, fully offline) — ships with the bundled `aiden` reference voice as the default, supports additional registered voices, and synthesizes long narration in `full` (one-pass) or `chunk` (per-sentence, individually regenerable) mode with tail-fade/pad, end-cutoff `audit`+`regen`, and a loudness-normalized edit track for video. macOS `say` fallback for quick non-cloned speech. | macOS (Apple Silicon), `mlx-audio` (`uv tool install mlx-audio`), `huggingface_hub` (`hf`, model weights), `ffmpeg` |
...
| `ffmpeg` | Local `ffmpeg`/`ffprobe` (+ macOS `sips`) media operations — cut/join/silence-cut/compress/convert, extract tracks, resize, FPS, rotate, GIFs, subtitles, thumbnails, audio normalization, HDR-to-SDR, HEIC, and bundled triple-pulse cut-marker timestamp detection. Stream-copy vs re-encode decision model plus recipe guides for tricky cuts/joins. | `ffmpeg`/`ffprobe` (macOS `sips` for HEIC) |
