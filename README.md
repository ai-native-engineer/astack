...
| `session-history` | Unified view and search of Claude Code, Codex, and Grok sessions, including timelines, full conversation/tool-call search, token usage, API-equivalent cost, model rollups, and live subscription quota. | Python 3 |
...
| `html-explainer` | Single-file HTML explainers in three types: visual diagrams/charts, interactive step-through walkthroughs, or searchable wiki/knowledge bases. Includes dark/light templates, UI polish rules, and headless render verification. | `chrome-devtools-cli` skill's CLI (for `verify.sh`), internet for CDNs (jsDelivr/unpkg) |
...
| `communication` | Shared work-message contract and channel-specific writing overlays used by transport skills. | None for drafting; the corresponding transport skill for read/send actions |
...
| `stt` | Local speech-to-text and optional speaker diarization. Uses macOS `apple-stt` as the text source, falls back to Handy only when Apple fails, and can align Apple text with Argmax RTTM speaker ranges. | macOS 26+ with `apple-stt` on PATH, `ffmpeg`; optional Handy and `argmax-cli` |
...
| `video-cut-editor` | Evidence-driven video cleanup for silence removal and retake markers, with reviewable cut plans, one-pass original-timeline rendering, frame/waveform/decode QA, and browser listening approval. | Python 3, `ffmpeg`/`ffprobe`; optional STT/VAD tools |
...
| `kakaotalk` | macOS KakaoTalk read, search, and send through the `katok` CLI, including archive sync, duplicate-room rejection, dry-run targeting, and draft-only delivery. | macOS, KakaoTalk app, Rust `katok` CLI, Full Disk Access; Accessibility for sending |
