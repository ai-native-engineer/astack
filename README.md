...
| `snowflake` | Snowflake SQL / scout / catalog — run queries (DataFrame/CSV/JSON), discover accessible DB/schema/table within permission boundaries, build schema sample catalogs. Account and key come from env, never hardcoded. | `uv` (injects `snowflake-connector-python`, `pandas`), Snowflake creds via env (`SNOWFLAKE_ACCOUNT`/`SNOWFLAKE_USER` + one auth method) |
